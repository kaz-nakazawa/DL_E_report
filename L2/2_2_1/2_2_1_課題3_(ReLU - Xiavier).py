...
def init_network():
    ...
    network ['W2'] = np.random.randn (hidden_layer_1_size, hidden_layer_2_size) / np.sqrt (
        hidden_layer_1_size) * np.sqrt (2)
    network ['W3'] = np.random.randn (hidden_layer_2_size, output_layer_size) / np.sqrt (
        hidden_layer_2_size) * np.sqrt (2)
    '''
    # Xavierの初期値
...
def forward(network, x):
    ...
    hidden_f = functions.relu # RELU関数
...
plt.title ("count - accuracy : ReLU - Xiavier")
plt.xlabel ("count")
plt.ylabel ("accuracy")
plt.ylim (0, 1.0)
# グラフの表示
